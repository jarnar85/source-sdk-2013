...
//-----------------------------------------------------------------------------
// Purpose: Make sure we wont be hating on our own creator
//-----------------------------------------------------------------------------
Disposition_t  CNPC_Manhack::IRelationType( CBaseEntity *pEnemy )
{
	if (pEnemy && pEnemy->IsPlayer() && pEnemy == GetOwnerEntity())
	{
		return D_LI;
	}

	return BaseClass::IRelationType( pEnemy );
}
